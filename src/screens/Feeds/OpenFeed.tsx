...
export default function OpenFeed({ navigation, route }) {
    ...
    useEffect(() => {
        // settweetid(tweetid.push(item.id))
        if (item) {
            const ss = []
            item.comments?.map((c) => {
    ...
    return (
        <>
            ...
            {visible === "comments" && <CustomPaperModal modalPage={tweet.length < 1&&{minHeight:"30%"}} hideModal={hideModal}>
                ...
                {
                    tweet.length > 1 ? (
                        <FlashList estimatedItemSize={tweet.length} data={tweet} renderItem={(props) => <Feeds navigation={navigation} {...props} />} />
            ...
            <View style={{ ...styles.FeedsActionContainer, width: "100%" }}>
                ...
                <View style={styles.FeedsActionRight}>
                    ...
                    <FeedsAction onPress={() => setVisible("comments")} count={"123"} name='message-circle' type='feather' />
                    <FeedsAction onPress={() => setVisible("share")} count={"123"} name='share-2' type='feather' />
                </View>
                <View style={{ marginTop: 10, display: "flex", flexDirection: "row", justifyContent: "space-between" }}>
                    <CustomComment lable='comment' />
                    <FeedsAction name='send' type='feather' />
                </View>
