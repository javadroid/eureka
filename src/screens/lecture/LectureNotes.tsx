...
export default function LectureNotes({ navigation }) {
    ...
    useEffect(() => {
        const gen = generateStrings("2020/CP/CSC", 12,4)
        console.log("gen", gen)
    }, [])
    ...
    function generateStrings(prefix: string, totalStrings: number,zero:number) {
        const strings = [];
      
        for (let i = 1; i <= totalStrings; i++) {
            const paddedNumber = String(i).padStart(zero, '0');
            strings.push(`${prefix}/${paddedNumber}`);
        }
